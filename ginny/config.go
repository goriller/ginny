...
func NewConfig(ops ...Options) (*Config, error) {
	c := &Config{}
...
// LoadEnv reads the given env file and extracts the variables as a string map
...
// readEnv opens the given env file and extracts the variables as a string map
func readEnv(file *os.File) (map[string]string, error) {
	items := map[string]string{}
	scanner := bufio.NewScanner(file)

	for scanner.Scan() {
		if key, value, err := parseEnv(scanner.Text()); err != nil {
			return nil, err
		} else if key != "" {
			items[key] = value
		}
...
// parseEnv extracts the key/value from the given line
func parseEnv(line string) (string, string, error) {
