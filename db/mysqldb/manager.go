...
func (m *Manager) Close() {
	...
	if err := m.writeDB.Close(); err != nil {
		logger.Error(fmt.Sprintf("close db write pool error: %s", err.Error()))
	}
	for i := 0; i < len(m.readDBs); i++ {
		if err := m.readDBs[i].Close(); err != nil {
			logger.Error(fmt.Sprintf("close db read pool error: %s", err.Error()))
		}
	}
}
	...
	for {
		...
		case <-ticker.C:
			if err := db.Ping(); err != nil {
				logger.Error(fmt.Sprintf("keepalive db ping error: %s", err.Error()))
			}
		}
	}
}
