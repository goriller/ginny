...
type Config struct {
	//该连接池可操作的数据库（可选配置）
	DBNames []string `json:"dbnames" yaml:"dbnames"`
	//初始化mongodb连接管理器时 给定默认数据库名（必须配置，否则业务方需注意不调用Manager.DB()函数）
	DefaultDBName string `json:"default_dbname" yaml:"default_dbname"`
	...
	ReadPreference struct {
		...
		MaxStaleness int `json:"max_staleness" yaml:"max_staleness"`
		//Name-Value 标签
		Tags map[string]string `json:"tags" yaml:"tags"`
	} `json:"read_preference" yaml:"read_preference"`

	//认证：支持用户名密码、AWS、GSSAPI/SSPI、LDAP、SCRAM。X509还需tls配置
	//详见 mongo/options/clientoptions.go Credential注释
	Auth struct {
		Mechanism           string            `json:"mechanism" yaml:"mechanism"`
		MechanismProperties map[string]string `json:"mechanism_properties" yaml:"mechanism_properties"`
